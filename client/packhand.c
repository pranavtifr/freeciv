			     ...
			     bool make_winner_veteran)
{
  ...
  create_event(unit_tile(punit1), E_UNIT_LOST_ATT, ftc_client,
               _("Combat result: defending %s (%d) %d hp (vet:%d, mp: %d(%d)), attacking %s"
                 "(%d) %d hp (vet: %d, mp: %d(%d)). (%s)"),
               unit_name_translation(punit1), defender_unit_id, defender_hp, punit1->veteran, punit1->moves_left, punit1->fuel, unit_name_translation(punit0), attacker_unit_id, attacker_hp, punit0->veteran, punit0->moves_left, punit0->fuel,
               make_winner_veteran?"Promoted":"Not Promoted");
